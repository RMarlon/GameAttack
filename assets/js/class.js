...
class Stage {
    ...
    doAttack(attacking, attacked) {
        if (attacking.life <= 0 || attacked.life <= 0) {
            this.log.addMessage('Attacking killed dog');
            return;
        }
        ...
        if(actualAttack > actualDefense){
            ...
            this.log.addMessage(`${attacking.name} caused ${actualAttack.toFixed(2)} of in damage ${attacking.name}`);
        }
        else{
            this.log.addMessage(`${attacked.name} defense conclued ...`);
        }
